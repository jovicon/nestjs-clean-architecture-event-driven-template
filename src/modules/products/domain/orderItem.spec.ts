...
describe('OrderItem (Products Module)', () => {
  describe('create', () => {
    ...
    it('should create OrderItem with unicode characters', () => {
      const props: OrderItemProps = {
        value: 'producto-日本語-émoji-🎉',
      };
      ...
      expect(result.isSuccess).toBe(true);
      const orderItem = result.getValue();
      expect(orderItem.value).toBe('producto-日本語-émoji-🎉');
    });

    it('should create OrderItem with whitespace', () => {
      const props: OrderItemProps = {
        value: '   spaced value   ',
      };
      ...
      expect(result.isSuccess).toBe(true);
      const orderItem = result.getValue();
      expect(orderItem.value).toBe('   spaced value   ');
    });
    ...
    it('should throw when props object is null', () => {
      expect(() => OrderItem.create(null as any)).toThrow();
    });

    it('should throw when props object is undefined', () => {
      expect(() => OrderItem.create(undefined as any)).toThrow();
    });
  });
  ...
  describe('ValueObject behavior', () => {
    ...
    it('should have immutable props via constructor pattern', () => {
      ...
      expect(orderItem.props).toBeDefined();
      expect(orderItem.props.value).toBe('immutable-test');
    });
  });
});
